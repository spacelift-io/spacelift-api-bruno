...
Fifteen requests used to carry a generated `example { }` block, showing the shape of a
response before you had credentials. They were removed: setup is now three fields in the
app and the pre-request script mints its own token, so sending the real request costs less
than reading a synthetic one, and an example built from the schema rather than an account
is one misreading away from looking like somebody's data. Don't add them back.
